...
| **Effective Value**                     | The value in force for a setting at a given scope: the override resolved via the inheritance walk if present, otherwise the Schema Default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| **Staged / Pending Change**             | A **value** change (set / revert / remove-value / clone) marked as not-yet-applied; it does not affect running services until Apply. Declaration operations (create / edit metadata / remove a setting) are **not** staged — they apply immediately.                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Setting Declaration**                 | The definition of a setting — key, GTS type, default, metadata, and Scope Class — distinct from its runtime value(s). A declaration is authored either by a platform administrator (admin-authored settings) or contributed by a gear (see Contributed Setting).                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
- **Role**: Reviews the audit trail of settings mutations and secret reveals to answer "who changed what, when."
- **Needs**: A queryable, masked audit history per setting/scope and a global audit view with no reveal path for secrets.
...
- **Integration Direction**: Outbound — Settings Service queries `tenant-resolver` synchronously on every cascading-inheritance resolution and scope-enforcement check; `tenant-resolver` does not call into Settings Service.
- **Availability Expectation**: Required at request time for tenant-scoped operations. If `tenant-resolver` is unavailable, tenant-scoped reads/writes **MUST** fail closed (rejected, never silently defaulted to platform scope); platform-scoped operations are unaffected.
...
| **Staged Change → Apply**                      | `p1`         | Applies to **value** changes: pending state, pending-changes view, apply preview, credential step-up; applying activates values by live-read (pull) with cache invalidation — a consumer needing more self-reacts; per-change reporting. Declaration create/edit/remove are immediate (not staged). |
...
| **Security, Secrets & Audit**                  | `p1`         | AuthN + access-level gating (via `authn-resolver`/`authz-resolver`); secret encrypt-at-rest + masking; step-up before apply; audit of all mutations and of secret reveals; feature/licence gating (via `license-resolver`).                              |
| **Standard/Advanced Mode**                     | `p2`         | Per-user persisted mode; mode-filtered reads; legible truncation of hidden advanced settings.                                                                                                                                                            |
| **Search & Discoverability**                   | `p2`         | Cross-field search (key/description/value/category) respecting scope, mode, and visibility; never exposes secrets.                                                                                                                                       |
...
- **Rationale**: A predictable category/setting lifecycle with no-orphan deletion and uniqueness guarantees is the structural foundation the rest of the service (typed values, staged apply, cascading overrides) depends on.
...
The system **MUST** validate every setting value (override or default) against its declared GTS type at creation or change time and **MUST** reject invalid values with a clear, field-level error; `format` keywords (e.g. `uri`, `ipv4`/`ipv6`) and trait-driven rules (e.g. cron dialect, regex-compiles) **MUST** be asserted, not treated as advisory. The system **MUST** include, or let a client resolve, a setting's type and resolved trait set when read, so the client can render the appropriate input and pre-validate; structured (object/array) values **MUST** be supported, not only scalar values. A setting whose type carries the `secret` trait **MUST** be stored encrypted at rest and **MUST** be masked in every read response; the plaintext **MUST NOT** be retrievable through any read, search, or audit path.
...
The system **MUST** let an administrator search settings by key, description, value, or category name and **MUST** return matches as a flat list with category breadcrumbs and an indication of which field matched. Search **MUST** respect the same scope, mode, and tenant-visibility filters as browsing, and **MUST NOT** expose secret values.

- **Rationale**: A single, global search is the primary discoverability mechanism that lets administrators reach any setting without learning the category tree.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-tenant-admin`
...
Operations on a **setting Declaration itself** — creating a setting, editing its metadata, or removing it — are **NOT** staged: they take effect immediately (they define/redefine the setting rather than change a value in force). Staging governs values, whose change would otherwise alter live behaviour; a declaration edit has no in-effect value to gate. Removing a setting is a **soft-delete (retire)**: the setting is marked retired and drops out of resolution at once, but its values are **retained** and recoverable by reactivation.

- **Rationale**: Decoupling "change" from "effect" is the core safety guarantee against accidental or unreviewed configuration changes to a live platform; declaration authoring has no in-effect value to gate, so it applies immediately rather than being staged.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-tenant-admin`
...
> **Note**: Revealing/using a secret value **MUST** be recorded as a security audit event (actor, timestamp, request id) with the value still masked in the record; routine reads are not audited, to bound log volume. Retention is configurable.
...
Given the platform is live and serving traffic, when setting **values** are changed but not yet applied, running services **MUST** be unaffected until an explicit, credential-verified Apply. A failed or partially-failed Apply **MUST** leave unapplied value changes in pending state for retry and **MUST** surface failure detail through a durable notification channel. Same-scope concurrent Apply attempts **MUST NOT** silently overwrite or reorder pending revisions; conflicts **MUST** be detected and reported. Conflict-handling mechanics are owned by DESIGN.
...
The system **MUST** enforce authentication and access-level gating on every operation, encrypt secrets at rest with masking on every read (no reveal path, including audit), require credential step-up before apply, audit all mutations, and enforce scope isolation server-side.

- **Threshold**: Zero unauthenticated operations; zero plaintext secret exposure through any path.
...
The system **MUST** maintain 99.9% monthly availability for the read path (effective-value resolution via the Settings read SDK). Administrative write/apply-path availability follows the platform's default maintenance-window expectations and is not held to a stricter bound in v1.

- **Threshold**: 99.9% monthly uptime for effective-value reads.
...
The system **MUST** support at least 100,000 tenants in the tenant scope hierarchy, a cascading-inheritance hierarchy depth of at least 10 levels, at least 5,000 distinct settings across all categories per platform instance (admin-authored and gear-contributed combined), and audit-record volume growing to at least 1,000,000 mutation events per tenant per year — all without degrading the read-path latency threshold ([§6.1 Performance: Read-Path Caching](#61-gear-specific-nfrs)) or Apply behavior.

- **Threshold**: 100,000 tenants; 10-level cascade depth; 5,000 settings per platform instance; 1,000,000 audit events/tenant/year — none of which degrades the stated read-latency or Apply thresholds.
- **Rationale**: Cascading-inheritance walk cost, gear-contributed declaration volume, and audit-record accumulation are the dimensions most likely to grow unbounded as more gears contribute settings and more tenants onboard.
...
- **Regulatory Compliance beyond stated audit requirements**: Not applicable as additional scope — audit coverage is fully defined by `cpt-cf-settings-service-nfr-security-baseline` and `cpt-cf-settings-service-fr-audit-mutations`, and PII-shaped values are governed by the data-classification model in [§5.2](#52-typed-values--validation). Audit-record actor identities (administrator user IDs, retention configurable per [§5.7](#57-security-secrets--audit)) are processed for accountability of privileged administrative actions (legitimate interest under GDPR Art. 6(1)(f)); if a gear-contributed setting's value is independently PII, its handling is governed by the [§5.2](#52-typed-values--validation) classification model. No further regulatory scope is introduced by this service.
...
- **Description**: Provides live-read access to a setting's effective value with cache invalidation on apply, and exposes the effective source / inheritance trail.
...
1. Administrator reads the setting's source indicator: own override / inherited from `<ancestor>` / Platform default
...
- [ ] `secret`-trait values stored encrypted at rest and masked in every read; plaintext not retrievable via any read, search, or audit path (`cpt-cf-settings-service-fr-typed-value-validation`)
...
- [ ] Cross-field search (key/description/value/category) returns a flat list with category breadcrumbs and matched-field indication; respects scope/mode/visibility filters; never exposes secrets (`cpt-cf-settings-service-fr-search-discoverability`)
...
- [ ] Declaration operations — create a setting, edit its metadata, remove it — are NOT staged and apply immediately; removal is a soft-delete (retire), with values retained and recoverable (`cpt-cf-settings-service-fr-staged-change-pending`)
...
- [ ] Every operation requires an authenticated session/token and an authorization decision (view = read; owner/admin = mutate/apply); Apply additionally requires credential step-up (`cpt-cf-settings-service-fr-authn-role-gating`)
...
- [ ] Security baseline enforced on every operation: authentication + access gating, secrets encrypted and masked with no reveal path (including audit), step-up before apply, all mutations audited, server-side scope isolation (`cpt-cf-settings-service-nfr-security-baseline`)
...
- [ ] 100,000 tenants, 10-level cascade depth, 5,000 settings per platform instance, and 1,000,000 audit events/tenant/year sustained without degrading the read-latency or Apply thresholds (`cpt-cf-settings-service-nfr-scale-growth`)
...
- A credential step-up (re-authentication) primitive is built — by `authn-resolver` or a Settings-Service-local flow — before Apply can enforce it; `authn-resolver`'s current API has no re-authentication method (see [§13 Open Questions](#13-open-questions)).
...
| Tenant Resolver outage | Tenant-scoped reads and mutations cannot establish the target hierarchy or enforce scope isolation | Affected tenant-scoped operations fail closed per the Tenant Resolver availability expectation ([§2.2](#22-system-actors)); outage detection and surfacing to platform operations are owned by DESIGN |
