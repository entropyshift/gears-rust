...
| **Barrier Seam**                        | The explicit, audited, downstream-only channel through which a barriered (isolated/unmanaged) tenant receives default, non-sensitive settings from its parent chain — marked declarations only, read-only, never `secret`-trait values ([§5.6](#56-multi-tenant-overrides--cascading-inheritance)).                                                              |
...
| **Barrier Default Seam**                       | `p2`         | Downstream-only: barriered tenants receive explicitly-marked, non-sensitive defaults from the parent chain via `tenant-resolver`; read-only, never `secret`-trait, enumerable and auditable.                                                              |
...
> **Phasing register**: v1 = this document. Deferred and out-of-scope capabilities above, together with review-deferred follow-ups, are tracked as repository issues labeled `settings-service` rather than absorbed into this document.
...
A setting value **MUST** be identified by `(declaration, scope)` where scope is `platform`, `tenant`, or `(tenant, subject_type, subject_id)`. `subject_type` **MUST** be a GTS-registered type and `subject_id` a GTS instance identifier; Scope Class **MUST** be definable per subject type; deletion of a subject **MUST** trigger cleanup of its values (generalizing the tenant-deletion cleanup subscription). The identity model, API shape, and uniqueness indexes **MUST** be subject-aware from v1; the *implementation* **MAY** phase (tenant scope first, subject scopes following) — but nothing may ship that would require a data migration to add subject scopes.

- **Rationale**: Per-subject configuration (integrations, hosts, workloads) is the established generalization path for this service; landing the identity model now makes it an additive increment instead of a migration.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-contributing-module`

#### Hierarchy Barriers and the Downstream Default Seam

- [ ] `p2` - **ID**: `cpt-cf-settings-service-fr-barrier-default-seam`

Hierarchy barriers (isolated/unmanaged tenants, as modeled by `tenant-resolver`) **MUST** block upward and lateral setting visibility. A barriered tenant **MAY** receive **default, non-sensitive** settings from its parent chain through an explicit barrier seam: only declarations explicitly marked barrier-traversable **MUST** resolve downward — as read-only defaults, never values whose type carries the `secret` trait or a sensitivity classification. Nothing **MUST** traverse upward or laterally through a barrier; the set of barrier-traversable declarations **MUST** be enumerable and every barrier-crossing resolution **MUST** be auditable.

- **Rationale**: Isolated tenants still need sane platform defaults; an explicit, marked, downstream-only seam provides them without weakening the isolation the barrier exists to guarantee.
- **Actors**: `cpt-cf-settings-service-actor-platform-admin`, `cpt-cf-settings-service-actor-tenant-admin`
